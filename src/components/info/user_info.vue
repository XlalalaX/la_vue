...
export default {
  ...
  setup(props) {
    ...
    const info = ref(state.friendMap.get(uid.value));
    const isFriend = ref(info.value == null ? false : true);

    onMounted(async () => {
      if (info.value == null) {
        isFriend.value = false;
        await axios.get(`http://${rootAddr}/user/user_show_info?uid=${uid.value}`).then(res=>{
          if(res.data.code!=0){
            ElMessage.error(`获取用户详情失败${res.data.msg}`)
            return
          }
          info.value = res.data.data
          axios.get(`http://${rootAddr}/user_not/user_face_url?uid=${uid.value}`)
              .then((response) => {
